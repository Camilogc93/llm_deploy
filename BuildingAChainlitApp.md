...
R. We want to support streaming because it's important for large files. We want to be able to stream the file in chunks and not have to wait until we're done processing the entire file.
...
R. We need to use a user session because we want to persist our chain across sessions in memory persist.
